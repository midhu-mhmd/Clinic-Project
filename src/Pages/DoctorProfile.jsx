...
import {
  Star, Shield, Clock, MapPin, ArrowRight,
  MessageSquare, Quote, Award, Loader2, AlertCircle
} from "lucide-react";
  ...
  useEffect(() => {
    const fetchDoctorDetails = async () => {
      try {
        ...
        /**
         * FIX: Pointing to the PUBLIC endpoint. 
         * No token header required for public viewing.
         */
        const { data } = await axios.get(`http://localhost:5000/api/doctors/public/${id}`);
        ...
        setError(err.response?.data?.message || "Clinical dossier could not be retrieved.");
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      gsap.from(".hero-text-reveal", {
        ...
        duration: 1.6, ease: "expo.out", stagger: 0.1,
      });

      gsap.fromTo(imageRef.current,
        { scale: 1.15, filter: "grayscale(100%)" },
        {
          scale: 1, filter: "grayscale(30%)",
          scrollTrigger: {
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#FAF9F6]">
        ...
        <p className="text-[10px] uppercase tracking-[0.4em] opacity-40">Decrypting Profile...</p>
      </div>
    );
  }
  ...
  if (error || !doctor) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#FAF9F6] text-red-500">
        ...
        <p className="text-[10px] uppercase tracking-[0.4em] mb-4">{error || "Doctor not found"}</p>
        <button onClick={() => window.history.back()} className="text-[9px] uppercase tracking-widest text-[#2D302D] underline">Return to Directory</button>
      </div>
    );
  }
      ...
      <section className="hero-section relative pt-40 pb-24 px-8 lg:px-16 grid grid-cols-1 lg:grid-cols-12 gap-16 items-end border-b border-[#2D302D]/5">
        <div className="lg:col-span-7">
          ...
          <p className="hero-text-reveal text-xl lg:text-2xl mt-10 font-light text-[#2D302D]/60 max-w-xl leading-relaxed italic">
            "{doctor.about || `Pioneering ${doctor.specialization} through clinical excellence and architectural restorative care.`}"
          </p>
        </div>
        ...
        <div className="lg:col-span-5 relative aspect-[4/5] overflow-hidden rounded-sm bg-[#2D302D]/5">
          <img
            ref={imageRef}
            src={doctor.image || `https://ui-avatars.com/api/?name=${doctor.name}&background=8DAA9D&color=fff&size=512`}
            alt={doctor.name}
        ...
        <div className="lg:col-span-7 space-y-32">
          <div className="grid grid-cols-3 border-y border-[#2D302D]/5 py-12">
            {[
              ...
              { label: "Clinic", val: doctor.tenantId?.name || "Global Network" },
            ].map((stat, i) => (
          ...
          <section className="fade-up space-y-10">
            ...
            <div className="text-2xl text-[#2D302D]/70 font-light leading-snug space-y-8">
              <p className="border-l-2 border-[#8DAA9D]/20 pl-8">
                Clinical practice in {doctor.specialization} is the pursuit of technical perfection balanced by
                human empathy. Dr. {doctor.name.split(' ').pop()} utilizes advanced protocols to ensure
                long-term patient resilience.
              </p>
              ...
              <div className="grid grid-cols-2 gap-8 pt-8">
                <div className="p-8 bg-white border border-[#2D302D]/5 flex flex-col gap-4">
                  ...
                  <span className="text-sm font-bold">Board of {doctor.specialization} Excellence</span>
                </div>
                <div className="p-8 bg-white border border-[#2D302D]/5 flex flex-col gap-4">
                  ...
                  <span className="text-sm font-bold">Verified Medical Faculty Member</span>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="fade-up pt-10 border-t border-[#2D302D]/5">
            <div className="relative p-16 bg-[#2D302D] text-[#FAF9F6]">
              ...
              <p className="relative z-10 text-2xl font-light italic leading-relaxed mb-10">
                ...
                A total shift in how I view medical care under Dr. {doctor.name.split(' ').pop()}."
              </p>
              <div className="flex items-center gap-6">
                <div className="w-12 h-px bg-[#8DAA9D]" />
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]">Patient Registry // Verified Review</span>
              </div>
            </div>
          </section>
        </div>
          ...
          <div className="sticky top-24">
            <div className="bg-[#2D302D] text-[#FAF9F6] p-12 shadow-2xl space-y-12">
              ...
              <div className="space-y-6">
                {[
                  { type: "In-Clinic Visit", price: `$${doctor.fees || '150'}`, icon: <MapPin size={16} /> },
                  { type: "Neural Consult (Video)", price: `$${Math.round((doctor.fees || 150) * 0.6)}`, icon: <MessageSquare size={16} />, active: true },
                ].map((opt, i) => (

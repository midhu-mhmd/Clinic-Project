...
import { Search, ArrowRight, MessageCircle, BookOpen, ShieldCheck, Cpu } from "lucide-react";
...
const Help = () => {
  ...
  const categories = [
    { title: "Protocol Initiation", count: "12 Guides", icon: <BookOpen size={20} />, id: "01" },
    { title: "Facility Infrastructure", count: "08 Guides", icon: <ShieldCheck size={20} />, id: "02" },
    { title: "Transaction Architecture", count: "05 Guides", icon: <Cpu size={20} />, id: "03" },
    { title: "Neural Assistant Setup", count: "10 Guides", icon: <MessageCircle size={20} />, id: "04" },
  ];

  return (
    <div ref={mainRef} className="min-h-screen bg-[#FAF9F6] text-[#2D302D] selection:bg-[#8DAA9D] selection:text-[#FAF9F6]">
          ...
          <div className="flex flex-col md:flex-row justify-between items-baseline gap-8 mb-16">
            <div className="overflow-hidden">
              <h1 className="reveal text-[clamp(3rem,8vw,7rem)] font-light leading-[0.9] tracking-tighter uppercase">
                Support <span className="italic font-serif text-[#8DAA9D] lowercase tracking-normal">Index.</span>
              </h1>
            </div>
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-12">
                    <span className="text-[10px] font-mono text-[#8DAA9D] tracking-widest">{cat.id} //</span>
                    <div className="opacity-20 group-hover:opacity-100 group-hover:text-[#8DAA9D] transition-all duration-500">
                      {cat.icon}
                    </div>
                  </div>
                  <h3 className="text-2xl font-light tracking-tight mb-2 group-hover:italic transition-all duration-500">{cat.title}</h3>
                  <p className="text-[9px] tracking-[0.3em] uppercase text-[#2D302D]/40 font-bold">{cat.count}</p>
                </div>
                ...
                <div className="flex items-center gap-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <span className="text-[9px] uppercase tracking-widest font-bold">Access Files</span>
      ...
      <section className="py-40 px-8">
        <div className="max-w-[1400px] mx-auto border-t border-[#2D302D]/5 pt-20 flex flex-col lg:flex-row justify-between items-end gap-16">
          <div className="reveal">
            <h4 className="text-[clamp(2rem,4vw,4rem)] font-light tracking-tighter leading-none mb-6">
              Require human <br />
              <span className="italic font-serif text-[#8DAA9D]">intervention?</span>
            </h4>
            <p className="text-[#2D302D]/40 text-sm font-light max-w-sm leading-relaxed">
              Our technical engineering team is available for high-priority architectural support 24/7 for Tier-1 members.
            </p>
          </div>

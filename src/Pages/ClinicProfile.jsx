...
const ClinicProfile = () => {
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#F0FDFA]">
        <Loader2 className="animate-spin text-[#0F766E] mb-4" size={48} />
  ...
  if (error) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#F0FDFA] px-6 text-center">
  ...
  return (
    <div
      ref={containerRef}
      className="bg-[#F0FDFA] text-[#1E293B] selection:bg-[#0F766E] selection:text-[#F0FDFA]"
    >
      {/* MIX-BLEND NAV */}
      <nav className="fixed top-0 left-0 w-full z-50 p-8 flex justify-between items-center pointer-events-none">
        <button
          onClick={() => navigate(-1)}
          className="pointer-events-auto flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold group text-[#F0FDFA] mix-blend-difference"
        >
      ...
      >
        <div className="absolute inset-0 z-0">
          <img
            ...
            alt={clinicName}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#F0FDFA] via-transparent to-transparent opacity-80" />
        </div>

        <div className="relative z-10 w-full flex flex-col md:flex-row justify-between items-end gap-12">
          <div className="max-w-4xl">
            <div className="flex items-center gap-4 mb-10 overflow-hidden">
              <Star size={14} className="text-[#0F766E] hero-reveal" />
              <span className="hero-reveal text-[10px] uppercase tracking-[0.5em] font-bold text-[#F0FDFA]/60">
                Official Exhibit — {String(clinic._id || "").slice(-6).toUpperCase()}
              </span>
            </div>

            <div className="overflow-hidden mb-2">
              <h1 className="hero-reveal text-[clamp(3.5rem,8vw,10rem)] font-light leading-[0.8] tracking-tighter uppercase italic font-serif text-[#F0FDFA]">
                {firstName}
              </h1>
            </div>
            <div className="overflow-hidden">
              <h1 className="hero-reveal text-[clamp(3.5rem,8vw,10rem)] font-light leading-[0.8] tracking-tighter uppercase text-[#0F766E]">
                {remainingName || "Facility"}
              </h1>
            </div>
          </div>

          <div className="flex flex-col items-end gap-10 text-[#F0FDFA] text-right">
            ...
            <div className="w-16 h-16 rounded-full border border-[#F0FDFA]/20 flex items-center justify-center hover:bg-[#F0FDFA] hover:text-[#1E293B] transition-all duration-700 cursor-pointer animate-bounce">
              <ArrowDown size={18} />
            </div>
          </div>
        </div>
        ...
        <div className="info-grid grid grid-cols-1 lg:grid-cols-12 gap-24">
          <div className="lg:col-span-7 space-y-32">
            <section className="info-block">
              <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-[#0F766E] mb-8 block">
                Philosophy
              </span>
              ...
              <p className="text-xl text-[#1E293B]/60 font-light leading-relaxed italic max-w-2xl">
                "{clinic.description || "Medical facility focused on clinical excellence."}"
              </p>
            </section>

            <div className="info-block grid grid-cols-1 sm:grid-cols-2 gap-px bg-[#1E293B]/10 border border-[#1E293B]/10">
              {[
                ...
                <div
                  key={i}
                  className="bg-[#F0FDFA] p-12 hover:bg-[#0F766E]/5 transition-colors group"
                >
                  <div className="text-[#0F766E] mb-6 opacity-40 group-hover:opacity-100 transition-all">
                    {item.icon}
                  </div>
          ...
          <div className="lg:col-span-5 relative">
            <div className="info-block sticky top-32 bg-[#1E293B] text-[#F0FDFA] p-12 space-y-12 rounded-sm shadow-2xl">
              <div className="flex justify-between items-center border-b border-[#F0FDFA]/10 pb-8">
              ...
              <div className="space-y-6">
                ...
                <p className="text-sm text-[#F0FDFA]/40 font-light italic leading-relaxed">
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="w-full bg-[#0F766E] text-[#F0FDFA] py-8 text-[10px] uppercase tracking-[0.5em] font-bold hover:bg-[#F0FDFA] hover:text-[#1E293B] transition-all duration-700"
                >
                  Book Appointment
                </button>

                <button className="w-full border border-[#F0FDFA]/10 py-8 text-[10px] uppercase tracking-[0.5em] font-bold hover:border-[#F0FDFA] transition-all duration-700">
                  Inquire
                </button>
              </div>
            </div>
      ...
      {/* TAGS FOOTER */}
      <section className="px-8 lg:px-16 py-20 border-t border-[#1E293B]/5">
        <div className="flex flex-wrap gap-4">
          {tags.length > 0 ? (
            tags.map((tag, i) => (
              <span
                key={`${tag}-${i}`}
                className="text-[10px] uppercase tracking-[0.3em] font-bold px-6 py-3 border border-[#1E293B]/10 rounded-full"
              >
                {tag}
              </span>
            ))

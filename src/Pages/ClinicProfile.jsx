...
const ClinicProfile = () => {
  ...
  useEffect(() => {
    const fetchClinicDetails = async () => {
      try {
        // Calling your updated GET /api/tenants/:id route
        const { data } = await axios.get(`http://localhost:5000/api/tenants/${id}`);
        setClinic(data);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      gsap.from(".hero-reveal", {
        y: 120, skewY: 5, opacity: 0, duration: 1.8, ease: "expo.out", stagger: 0.1,
      });
      ...
      gsap.from(".info-block", {
        opacity: 0, y: 40, duration: 1.2, stagger: 0.1,
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#FAF9F6]">
        ...
        <p className="text-[10px] uppercase tracking-widest opacity-40">Decrypting Facility Data</p>
      </div>
    );
  }

  // Handle dynamic name splitting for the "Editorial" look
    ...
    <div ref={containerRef} className="bg-[#FAF9F6] text-[#2D302D] selection:bg-[#8DAA9D] selection:text-[#FAF9F6]">
      ...
      <section ref={heroRef} className="relative h-[90vh] w-full overflow-hidden flex items-end pb-24 px-8 lg:px-16">
        <div className="absolute inset-0 z-0">
          <img
            src={clinic.img || "https://images.unsplash.com/photo-1629909613654-2871b886daa4"}
            className="hero-img w-full h-full object-cover grayscale-[0.3] brightness-[0.5] scale-110"
            alt={clinic.name}
          />
          <div className="absolute inset-0 bg-linear-to-t from-[#FAF9F6] via-transparent to-transparent opacity-80" />
        </div>
        ...
        <div className="relative z-10 w-full flex flex-col md:flex-row justify-between items-end gap-12">
          <div className="max-w-4xl">
            ...
            <div className="overflow-hidden mb-2">
              <h1 className="hero-reveal text-[clamp(4rem,10vw,10rem)] font-light leading-[0.8] tracking-tighter uppercase italic font-serif text-[#FAF9F6]">
                {firstName}
              </h1>
            </div>
            <div className="overflow-hidden">
              <h1 className="hero-reveal text-[clamp(4rem,10vw,10rem)] font-light leading-[0.8] tracking-tighter uppercase text-[#8DAA9D]">
          ...
          <div className="flex flex-col items-end gap-10 text-[#FAF9F6] text-right">
            <p className="max-w-70 text-[11px] uppercase tracking-widest font-bold opacity-70 leading-relaxed">
              Established in {clinic.location}. <br /> Verified Premium Clinical Infrastructure.
            </p>
        ...
        <div className="info-grid grid grid-cols-1 lg:grid-cols-12 gap-24">
          ...
          <div className="lg:col-span-7 space-y-32">
            ...
            <div className="info-block grid grid-cols-2 gap-px bg-[#2D302D]/10 border border-[#2D302D]/10">
              {[
                { label: "Address", val: clinic.location, icon: <MapPin size={14} /> },
                { label: "Availability", val: "By Appointment", icon: <Clock size={14} /> },
                { label: "Standard", val: "Tier-1 Certified", icon: <ShieldCheck size={14} /> },
          ...
          <div className="lg:col-span-5 relative">
            <div className="info-block sticky top-32 bg-[#2D302D] text-[#FAF9F6] p-12 space-y-12 rounded-sm shadow-2xl">
              ...
              <div className="space-y-6">
                <h3 className="text-4xl font-light tracking-tighter uppercase">Reserve Slot</h3>
                <p className="text-sm text-[#FAF9F6]/40 font-light italic leading-relaxed">
                  Instant scheduling is active for this facility. Please ensure you have your medical ID ready for verification.
                </p>
              </div>
      ...
      <section className="px-8 lg:px-16 py-20 border-t border-[#2D302D]/5">
        <div className="flex flex-wrap gap-4">
          {clinic.tags?.map((tag, i) => (
            <span key={i} className="text-[10px] uppercase tracking-[0.3em] font-bold px-6 py-3 border border-[#2D302D]/10 rounded-full">
              {tag}
            </span>
          ))}
        </div>

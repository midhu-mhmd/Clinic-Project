...
const STATUS_MAP = {
  ...
  CANCELLED: { color: "text-[#2D302D]/40", bg: "bg-[#2D302D]/5", label: "Cancelled" },
};
      ...
      <div className="min-h-screen bg-[#FAF9F6] text-[#2D302D] pt-32 pb-20 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            {/* Header */}
            <div className="border-b border-[#2D302D]/5 pb-8">
              ...
              <h1 className="text-3xl font-light tracking-tight uppercase">
                Video <span className="italic font-serif text-[#8DAA9D]">Consultation</span>
              </h1>
            </div>

            {/* Details Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white border border-[#2D302D]/5 p-6">
                <span className="text-[9px] uppercase tracking-[0.3em] font-bold text-[#2D302D]/30 block mb-2">Doctor</span>
                <p className="text-sm font-medium">{s.doctorId?.name || "–"}</p>
                <p className="text-[10px] text-[#8DAA9D] mt-1">{s.doctorId?.specialization || ""}</p>
              </div>
              <div className="bg-white border border-[#2D302D]/5 p-6">
                <span className="text-[9px] uppercase tracking-[0.3em] font-bold text-[#2D302D]/30 block mb-2">Scheduled</span>
                <p className="text-sm font-medium">{formatDate(s.appointmentId?.dateTime)}</p>
              </div>
              <div className="bg-white border border-[#2D302D]/5 p-6">
                <span className="text-[9px] uppercase tracking-[0.3em] font-bold text-[#2D302D]/30 block mb-2">Duration</span>
                <p className="text-sm font-medium">{formatDuration(s.duration)}</p>
              </div>
              <div className="bg-white border border-[#2D302D]/5 p-6">
                <span className="text-[9px] uppercase tracking-[0.3em] font-bold text-[#2D302D]/30 block mb-2">Fee</span>
            ...
            {/* Doctor Notes */}
            {s.doctorNotes && (
              <div className="bg-white border border-[#8DAA9D]/20 p-6">
                <span className="text-[9px] uppercase tracking-[0.3em] font-bold text-[#8DAA9D] mb-3 flex items-center gap-2">
                  <FileText size={12} /> Doctor Notes
                </span>
                <p className="text-sm text-[#2D302D]/70 leading-relaxed whitespace-pre-wrap">{s.doctorNotes}</p>
              </div>
            )}
            ...
            {s.prescription && (
              <div className="bg-white border border-[#8DAA9D]/20 p-6">
                ...
                <p className="text-sm text-[#2D302D]/70 leading-relaxed whitespace-pre-wrap">{s.prescription}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <h1 className="text-5xl font-light tracking-tighter uppercase leading-none">
              My <span className="italic font-serif text-[#8DAA9D]">Consultations</span>
            </h1>
        ...
        {consultations.length === 0 ? (
          <div className="py-24 flex flex-col items-center justify-center text-center bg-white border border-[#2D302D]/5 rounded-[40px]">
            <Video size={40} className="text-[#2D302D]/10 mb-4" />
          ...
          <div className="space-y-3">
            {consultations.map((c) => {
              ...
              return (
                <div
                  key={c._id}
                  className="consult-card group bg-white border border-[#2D302D]/5 p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-[#8DAA9D]/20 hover:shadow-lg transition-all duration-500 cursor-pointer"
                  onClick={() => setSelectedSession(c)}
                >
                  <div className="flex items-start gap-5">
                    <div className="w-12 h-12 rounded-full bg-[#8DAA9D]/10 flex items-center justify-center shrink-0">
                      <Video size={20} className="text-[#8DAA9D]" />
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] text-[#8DAA9D]">{c.doctorId?.specialization}</p>
                  ...
                  {canJoin && (
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-3 bg-[#2D302D] text-white text-[9px] uppercase tracking-[0.2em] font-bold rounded-full hover:bg-[#8DAA9D] transition-all duration-500 shrink-0"
                    >

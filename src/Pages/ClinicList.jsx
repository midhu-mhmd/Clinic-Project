    ...
    <div ref={containerRef} className="bg-[#FAF9F6] text-[#2D302D] min-h-screen">
      ...
      <nav className="flex justify-between items-center px-8 py-8 border-b border-[#2D302D]/5">
        ...
        <div className="flex gap-12 text-[9px] uppercase tracking-[0.3em] font-bold opacity-40">
          <button className="hover:text-[#8DAA9D] hover:opacity-100 transition-all underline-offset-4 hover:underline">Filters</button>
          <button className="hover:text-[#8DAA9D] hover:opacity-100 transition-all underline-offset-4 hover:underline">Map View</button>
        </div>
      </nav>
      ...
      <main className="px-8 py-24">
        ...
        <header className="mb-32">
          <div className="overflow-hidden mb-4">
            <h1 className="reveal-text text-[clamp(3rem,9vw,9rem)] font-light leading-[0.85] tracking-tighter uppercase">
              Elite <span className="italic font-serif text-[#8DAA9D] lowercase tracking-normal">medical</span>
            </h1>
          </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-end mt-16 gap-12">
            <p className="max-w-md text-[#2D302D]/50 font-light text-lg italic leading-relaxed">
              "Access a curated selection of world-class clinics verified for clinical excellence and rigorous safety protocols."
            </p>
        ...
        <section className="clinic-grid grid grid-cols-1 md:grid-cols-2 gap-px bg-[#2D302D]/10 border border-[#2D302D]/10">
          {CLINICS.map((clinic) => (
            <div key={clinic.id} className="clinic-item bg-[#FAF9F6] p-12 group cursor-pointer relative transition-colors duration-700 hover:bg-[#8DAA9D]/5">
              
              <div className="flex justify-between items-start mb-20">
                <span className="text-[10px] font-mono opacity-30 uppercase tracking-widest">Exhibit — {clinic.id}</span>
              ...
              <div className="flex flex-col xl:flex-row gap-12 items-end">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {clinic.tags.map(tag => (
                      <span key={tag} className="text-[9px] uppercase tracking-widest font-bold px-4 py-2 border border-[#2D302D]/10 rounded-full group-hover:border-[#8DAA9D]/30 transition-colors">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mt-12 pt-8 border-t border-[#2D302D]/5 flex justify-between items-center opacity-40 group-hover:opacity-100 transition-opacity">
                <span className="text-[10px] uppercase tracking-[0.3em] font-bold">{clinic.location}</span>
      ...
      <footer className="px-8 py-40 text-center border-t border-[#2D302D]/5">
        <span className="text-[10px] tracking-[0.5em] text-[#8DAA9D] uppercase font-bold block mb-8">Ready to proceed?</span>
        <h3 className="text-[clamp(2.5rem,7vw,6rem)] font-light tracking-tighter uppercase mb-16 leading-none">
          Can't decide <br />
          <span className="italic font-serif text-[#8DAA9D]">on a facility?</span>
        </h3>

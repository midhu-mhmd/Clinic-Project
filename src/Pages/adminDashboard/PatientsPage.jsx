...
// Normalize role safely (handles undefined/null + different casing)
...
const PatientsPage = () => {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        if (!token) {
          setError("AUTH ERROR: No token found. Please login again.");
          return;
        }

        // NOTE: You are calling /users/all (we will filter client-side to PATIENT)
  ...
  // ✅ Patient-only list (role-based filter)
  const patientOnly = useMemo(() => {
    return patients.filter((p) => normalizeRole(p.role) === "PATIENT");
  }, [patients]);

  // ✅ Patient-only + search filter
  const filteredPatients = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();

    if (!q) return patientOnly;

    return patientOnly.filter((p) => {
      const name = (p.name || "").toLowerCase();
      const id = String(p.id || "").toLowerCase();
      const patientId = String(p.patientId || "").toLowerCase();

      return name.includes(q) || id.includes(q) || patientId.includes(q);
    });
  }, [patientOnly, searchQuery]);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#FBFBF9] selection:bg-[#8DAA9D] selection:text-white">
      ...
      <nav className="fixed top-0 w-full z-50 flex justify-between items-center px-8 md:px-16 py-10 pointer-events-none">
        ...
        <div className="pointer-events-auto flex gap-4">
          <button className="h-12 px-6 rounded-full bg-white/5 border border-white/10 flex items-center gap-3 hover:bg-white/10 transition-all group">
            <Filter
              size={16}
              className="opacity-40 group-hover:text-[#8DAA9D] transition-colors"
            />
          ...
          <AnimatePresence mode="popLayout">
            {filteredPatients.length > 0 ? (
              filteredPatients.map((patient, index) => (
                <PatientCard
                  key={patient._id || patient.id || `${patient.name}-${index}`}
                  patient={patient}
                  index={index}
                />
              ))
      ...
      <footer className="py-20 px-6 border-t border-white/5 text-center">
        <p className="text-[10px] font-bold uppercase tracking-[1em] opacity-30">
          Biometric Encryption Active — 2026
        </p>
      </footer>
    </div>
  );
    ...
    >
      <div
        className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent ${
          isCritical ? "via-red-500" : "via-[#8DAA9D]"
      ...
      <div>
          ...
          <div className="flex flex-col">
            ...
            <span
              className={`text-[9px] font-bold uppercase tracking-widest mt-1 ${
                isCritical ? "text-red-400" : "text-[#8DAA9D]"
              }`}
            >
            ...
            <img
              src={
                ...
                `https://ui-avatars.com/api/?name=${encodeURIComponent(
                  fullName
                )}&background=1a1a1a&color=8DAA9D`
              }
              alt={fullName}
        ...
        <h4 className="text-4xl font-light tracking-tighter mb-10 group-hover:text-[#8DAA9D] transition-colors leading-[0.85]">
          {firstName} <br />
          <span className="italic font-serif opacity-60">{restName}</span>
        </h4>
        ...
        <div className="grid grid-cols-2 gap-y-8 pt-8 border-t border-white/5">
          ...
          <div className="space-y-1">
            ...
            <p
              className={`text-xl font-light ${
                String(patient.blood || "").includes("-") ? "text-orange-300" : ""
              }`}
            >
              {patient.blood || "N/A"}
            </p>
          ...
          <div className="space-y-1 col-span-2">
            ...
            <p className="text-sm font-mono opacity-80">
              {patient.lastVisit ||
                (patient.updatedAt ? String(patient.updatedAt).split("T")[0] : "") ||
                "No Data"}
            </p>
          </div>
        </div>
...
const ErrorMessage = ({ message }) => (
  <div className="h-screen w-full bg-[#0A0A0A] flex flex-col items-center justify-center p-6">
    ...
    <span className="text-[11px] font-bold uppercase tracking-[0.3em] text-red-500/80">
      {message}
    </span>
    <button
...
const CinematicLoader = () => (
  <div className="h-screen w-full bg-[#0A0A0A] flex flex-col items-center justify-center gap-8">
    ...
    <span className="text-[10px] font-bold uppercase tracking-[1em] text-[#8DAA9D] animate-pulse">
      Syncing Biological Archive
    </span>
  </div>
);

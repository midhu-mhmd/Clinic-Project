...
const Doctors = () => {
  ...
  // Field 'fee' renamed to 'consultationFee' to match Mongoose Schema
  ...
  const resetForm = () => {
    ...
    setFormData({ 
      ...
      consultationFee: "", status: "On Duty" 
    });
    setImageFile(null);
  ...
  const handleEditInit = (doc) => {
    ...
    const [start, end] = doc.availability ? doc.availability.split(" - ") : ["09:00", "17:00"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Mapping form fields to match Backend Schema exactly
      data.append("name", formData.name);
      data.append("specialization", formData.specialization);
      data.append("email", formData.email);
      data.append("education", formData.education);
      data.append("experience", Number(formData.experience));
      data.append("availability", availabilityString);
    ...
    <div className="p-8 lg:p-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-gray-400 text-[11px] uppercase tracking-[0.2em]">Manage clinical practitioners for {doctors.length} active staff</p>
        </div>
        <button onClick={() => { resetForm(); setIsModalOpen(true); }} className="flex items-center gap-3 bg-black text-white px-6 py-4 text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-[#8DAA9D] transition-all shadow-xl">
            ...
            {successMsg ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="flex flex-col items-center mb-6">
                  ...
                  <div onClick={() => fileInputRef.current.click()} className="group relative w-24 h-24 rounded-full bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center cursor-pointer overflow-hidden transition-all hover:border-black">
                    {imagePreview ? <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" /> : <Camera size={24} className="text-gray-300 group-hover:text-black" />}
                  </div>
                  <p className="text-[9px] uppercase tracking-widest text-gray-400 mt-2 font-bold">Profile Image</p>
                </div>

                {['name', 'specialization', 'email', 'education', 'experience'].map((key) => (
                  <div key={key}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[9px] uppercase tracking-widest font-bold mb-1 block text-gray-400">Duty Start</label>
                    <input type="time" className="w-full border-b border-gray-200 py-2 focus:border-black outline-none text-sm" value={formData.availabilityStart} onChange={(e) => setFormData({...formData, availabilityStart: e.target.value})} />
                  </div>
                  <div>
                    <label className="text-[9px] uppercase tracking-widest font-bold mb-1 block text-gray-400">Duty End</label>
                    <input type="time" className="w-full border-b border-gray-200 py-2 focus:border-black outline-none text-sm" value={formData.availabilityEnd} onChange={(e) => setFormData({...formData, availabilityEnd: e.target.value})} />
                  </div>
                </div>

                <div>
                  <label className="text-[9px] uppercase tracking-widest font-bold mb-1 block text-gray-400">Consultation Fee ($)</label>
                  <input type="number" required className="w-full border-b border-gray-200 py-2 focus:border-black outline-none text-sm font-mono" placeholder="0.00" value={formData.consultationFee} onChange={(e) => setFormData({...formData, consultationFee: e.target.value})} />
                </div>
        ...
        {doctors.map((doc) => (
          <div key={doc._id} className="bg-white border border-gray-100 p-8 group hover:border-black transition-all duration-500 relative flex flex-col justify-between">
            <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
              <button onClick={() => handleEditInit(doc)} className="p-2 border border-gray-100 hover:border-black shadow-sm"><Edit3 size={14} /></button>
            ...
            <div className="flex gap-6 mb-6">
              ...
              <div>
                ...
                <div className="flex flex-col gap-1 text-gray-400 text-[10px] uppercase tracking-tight">
                  <span className="flex items-center gap-2"><Clock size={12}/> {doc.availability || "09:00 - 17:00"}</span>
                  <span className="flex items-center gap-2"><DollarSign size={12}/> ${doc.consultationFee || "0.00"} Consultation</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-gray-50 pt-6 mt-auto">
              <div className="text-[9px] uppercase tracking-widest text-gray-400">Experience <p className="text-sm font-semibold text-black">{doc.experience}Y</p></div>
